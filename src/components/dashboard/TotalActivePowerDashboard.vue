  ...
  methods: {
    updateChart () {
      ...
      if (this.selectedTransductor !== undefined) {
        axios
          .get(`http://localhost:8001/graph/minutely_total_active_power/?limit=${limit}&serial_number=${this.selectedTransductor}&start_date=${startDate}&end_date=${endDate}`)
          .then((res) => {
    ...
    buildGraphInformation (measurements) {
      ...
      for (let measurement of measurements) {
        ...
        // index of measurement activePower
        activePower = measurement.measurements[0][0]
        formattedDates.push(date)

        activePowerList.push(activePower)
      }

      this.setInformations(activePowerList, formattedDates)
    },

    setInformations (activePowerList, formattedDates) {
      this.active_power = activePowerList
