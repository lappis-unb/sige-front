...
export default {
  ...
  data() {
    return {
      ...
      optionsModel: null,
      optionsGroup: [],
      startDate: '02/2021', // Valor padrão por enquanto(unico mes com dados)
      mask: '##/####',
      adjustValue: false,
    };
  },
  ...
  methods: {
    ...
    async getReports() {
      try {
        const response = await report.getReports(this.formatDate(this.startDate), this.campusModel);
    ...
    async getUfer() {
      try {
        const response = await report.getUfer(this.formatDate(this.startDate), this.campusModel, 1); 
        this.response = response;
    ...
    async filterOptions(val, update) {
      ...
      update(() => {
        this.optionsCampus = this.optionsCampus.filter(v => v.name.toLowerCase().indexOf(needle) > -1);
    ...
    async getGroups() {
      const selectedCampus = this.optionsCampus.find(campus => campus.id === this.campusModel);
      if (selectedCampus) {
        const updatedGroups = selectedCampus.groups_related.filter((group, index, self) => {
    ...
    handleSwitch() {
      if (document.getElementById('subtitle').innerHTML == 'Ajustar para datas mensais') {
        document.getElementById('subtitle').innerHTML = 'Ajustar para datas de faturamento';
      }
      else {
        document.getElementById('subtitle').innerHTML = 'Ajustar para datas mensais';
      }
    },
