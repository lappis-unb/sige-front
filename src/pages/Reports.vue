<template>
  <div class="q-pa-md">
    <ReportFilter ref="ReportFilter" @data-updated="getReportData" @ufer-updated="getPowerFactorData" />
    <q-table
      :data="data"
...
export default {
  ...
  methods: {
    ...
    createReportChart() {
      ...
      const selectedDate = Object.keys(response)[0];
      ...
      try {
        const {
          ...
          value_peak,
          value_off_peak,
          total_consumption_off_peak_time = value_off_peak*consumption_off_peak_time,
          total_consumption_peak_time = value_peak*consumption_peak_time,
          total_generated_energy_off_peak_time = undefined,
          total_generated_energy_peak_time = undefined,
          total_valuekwh = (consumption_off_peak_time + consumption_peak_time)*-1 + (generated_energy_off_peak_time + generated_energy_peak_time),
          total_coast = (total_consumption_off_peak_time + total_consumption_peak_time) //+ (total_generated_energy_off_peak_time + total_generated_energy_peak_time)*-1,
        ...
        this.data = [
          {
            ...
            tarifa: Intl.NumberFormat('pt-BR').format(value_peak),
            total: Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(total_consumption_peak_time)
          },
          {
            name: "Consumo (Fora de Ponta)",
            valor_kwh: Intl.NumberFormat('pt-BR').format(consumption_off_peak_time),
            tarifa: Intl.NumberFormat('pt-BR').format(value_off_peak),
            total:  Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(total_consumption_off_peak_time)
          },
          {
            name: "Energia Gerada (Ponta)",
            valor_kwh:Intl.NumberFormat('pt-BR').format(generated_energy_peak_time), 
            tarifa: undefined,
            total: total_generated_energy_peak_time
          },
          {
            name: "Energia Gerada (Fora de Ponta)",
            valor_kwh:Intl.NumberFormat('pt-BR').format(generated_energy_off_peak_time),
            tarifa: undefined,
            total: total_generated_energy_off_peak_time
          },
          {
            name: "Total",
            valor_kwh:Intl.NumberFormat('pt-BR').format(total_valuekwh)+ " kWh" ,
            tarifa: undefined,
            total: Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(total_coast)
          }
  ...
  data() {
    return {
      ...
      reportColumns: [
        ...
        { name: 'valor_kwh', align: 'right', label: 'Valor (kWh)', field: 'valor_kwh' },
